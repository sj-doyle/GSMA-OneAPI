...
<?php 
	...
	if (!isset($submit)) {
		...
		$username="Fred.Jones";
		$password="1234";
		$endpoint=$endpoints->getMMSDeliverySubscriptionsEndpoint();
	} else {		
		$endpoints->setMMSDeliverySubscriptionsEndpoint($endpoint);

		$sender = new MMSSend($endpoints, $username, $password);
	
		$mmsResponse = $sender->subscribeToDeliveryNotifications($senderAddress, $clientCorrelator, $notifyURL, $callbackData);	
	}
...
<div id="content">
	...
	<?php include("mainmenu.inc"); ?>
		...
		<form method="post" action="mmssubscribedeliveryreceipts.php">
        ...
        <?php if (isset($mmsResponse)) { ?>
          <h3>Response:</h3>
			...
			<?php if (!is_null($mmsResponse->getRequestError())) { ?>
				...
				<?php } ?>
			<?php } ?>
			<?php if (!is_null($mmsResponse->getDeliveryReceiptSubscription())) { ?>
          		<div class="ParameterGroupHeading"><strong>DeliveryReceiptSubscription</strong></div>
				<?php if (!is_null($mmsResponse->getDeliveryReceiptSubscription()->getCallbackReference())) { ?>
		          	<div class="ParameterRow">
						<div class="ParameterLabel">callbackReference.callbackData</div>
						<div class="ParameterValue"><?php echo $mmsResponse->getDeliveryReceiptSubscription()->getCallbackReference()->getCallbackData();?></div>				
					</div>
		          	<div class="ParameterRow">
						<div class="ParameterLabel">callbackReference.notifyURL</div>
						<div class="ParameterValue"><?php echo $mmsResponse->getDeliveryReceiptSubscription()->getCallbackReference()->getNotifyURL();?></div>				
					</div>
				<?php } ?>
	          	<div class="ParameterRow">
					<div class="ParameterLabel">resourceURL</div>
					<div class="ParameterValue"><?php echo $mmsResponse->getDeliveryReceiptSubscription()->getResourceURL();?></div>				
				</div>
			<?php 
			} 
		} ?>
