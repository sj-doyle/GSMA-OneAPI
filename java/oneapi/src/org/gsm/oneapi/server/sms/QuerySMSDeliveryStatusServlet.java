...
import org.gsm.oneapi.responsebean.sms.DeliveryInfoList;
import org.gsm.oneapi.server.OneAPIServlet;
import org.gsm.oneapi.server.ValidationRule;

/**
 * Servlet implementing the OneAPI function for querying (sent) SMS delivery status
 */
public class QuerySMSDeliveryStatusServlet extends OneAPIServlet {
	...
	private final String[] validationRules={"1", "smsmessaging", "outbound", "*", "requests", "*", "deliveryInfos"};
	...
	public void doGet(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException{		
		...
		if (validateRequest(request, response, requestParts, validationRules)) {
			...
			if (checkRequestParameters(response, rules)) {
				...
				String resourceURL=getRequestHostnameAndContext(request)+request.getServletPath()+"/1/smsmessaging/outbound/"+urlEncode(senderAddress)+"/requests/"+urlEncode(requestId)+"/deliveryInfos"; 
