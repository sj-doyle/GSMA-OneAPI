...
<%@ page import="org.gsm.oneapi.responsebean.mms.MMSDeliveryReceiptSubscriptionResponse"%>
...
<%
	...
	MMSDeliveryReceiptSubscriptionResponse mmsResponse=null;
	...
	if (submit==null) {
		...
		username="Fred.Jones";
		password="1234";
		endpoint=sandboxEndpoints.getServiceEndpoints().getMMSDeliverySubscriptionsEndpoint();

	} else {
		sandboxEndpoints.setMMSDeliverySubscriptions(endpoint);
	...
	<%@ include file="mainmenu.inc" %>
        ...
        <div id="other-main">
          ...
          <div class="other-bottom">
            <div class="left">
          <h3>
            Outbound MMS API - Subscribe To MMS Delivery Notifications
          </h3>
          <p>
      
		</p>
		
		<form method="post" action="mmssubscribedeliveryreceipts.jsp">
          ...
          <h3>Response:</h3>
			...
			<% if (mmsResponse.getRequestError()!=null) { %>
				...
				<% } %>
			<% } %>
			<% if (mmsResponse.getDeliveryReceiptSubscription()!=null) { %>
          		<div class="ParameterGroupHeading"><strong>deliveryReceiptSubscription</strong></div>
          		<% if (mmsResponse.getDeliveryReceiptSubscription().getCallbackReference()!=null) { %>
		          	<div class="ParameterRow">
						<div class="ParameterLabel">callbackReference.callbackData</div>
						<div class="ParameterValue"><%=mmsResponse.getDeliveryReceiptSubscription().getCallbackReference().getCallbackData()%></div>				
					</div>
		          	<div class="ParameterRow">
						<div class="ParameterLabel">callbackReference.notifyURL</div>
						<div class="ParameterValue"><%=mmsResponse.getDeliveryReceiptSubscription().getCallbackReference().getNotifyURL()%></div>				
					</div>
				<% } %>
	          	<div class="ParameterRow">
					<div class="ParameterLabel">resourceURL</div>
					<div class="ParameterValue"><%=mmsResponse.getDeliveryReceiptSubscription().getResourceURL()%></div>				
				</div>
			<% } %>
