...
<?php 
	...
	if (!isset($submit)) {
		...
		$username="Fred.Jones";
		$password="1234";
		$endpoint=$endpoints->getRetrieveSMSEndpoint();
	} else {		
		$endpoints->setRetrieveSMSEndpoint($endpoint);

		$receiver = new SMSRetrieve($endpoints, $username, $password);
	
		$smsResponse = $receiver->retrieveMessages($registrationId, $maxBatchSize);	
	}
...
<div id="content">
	...
	<?php include("mainmenu.inc"); ?>
		...
		<form method="post" action="smsretrieve.php">
        ...
        <?php if (isset($smsResponse)) { ?>
          <h3>Response:</h3>
			...
			<?php if (!is_null($smsResponse->getRequestError())) { ?>
				...
				<?php } ?>
			<?php } ?>
			<?php if (!is_null($smsResponse->getInboundSMSMessageList())) {
				$inboundSMSMessage = $smsResponse->getInboundSMSMessageList()->getInboundSMSMessage(); 
				$count=0;
				if (!is_null($inboundSMSMessage)) $count=count($inboundSMSMessage);
			?>
          		...
          		<div class="ParameterRow">
					...
					<div class="ParameterValue"><?php echo $smsResponse->getInboundSMSMessageList()->getNumberOfMessagesInThisBatch(); ?></div>				
				</div>
          		<div class="ParameterRow">
					<div class="ParameterLabel">resourceURL</div>
					<div class="ParameterValue"><?php echo $smsResponse->getInboundSMSMessageList()->getResourceURL(); ?></div>				
				</div>
          		<div class="ParameterRow">
					<div class="ParameterLabel">totalNumberOfPendingMessages</div>
					<div class="ParameterValue"><?php echo $smsResponse->getInboundSMSMessageList()->getTotalNumberOfPendingMessages(); ?></div>				
				</div>
				...
				<?php for ($i=0; $i<$count; $i++) { ?>
		          	...
		          	<div class="ParameterRow">
						...
						<div class="ParameterValue"><?php echo $inboundSMSMessage[$i]->getDateTime();?></div>				
					</div>
		          	<div class="ParameterRow">
						<div class="ParameterLabel">destinationAddress [<?php echo $i; ?>]</div>
						<div class="ParameterValue"><?php echo $inboundSMSMessage[$i]->getDestinationAddress();?></div>				
					</div>
		          	<div class="ParameterRow">
						<div class="ParameterLabel">messageId [<?php echo $i; ?>]</div>
						<div class="ParameterValue"><?php echo $inboundSMSMessage[$i]->getMessageId();?></div>				
					</div>
		          	<div class="ParameterRow">
						<div class="ParameterLabel">message [<?php echo $i; ?>]</div>
						<div class="ParameterValue"><?php echo $inboundSMSMessage[$i]->getMessage();?></div>				
					</div>
		          	<div class="ParameterRow">
						<div class="ParameterLabel">resourceURL [<?php echo $i; ?>]</div>
						<div class="ParameterValue"><?php echo $inboundSMSMessage[$i]->getResourceURL();?></div>				
					</div>
		          	<div class="ParameterRow">
						<div class="ParameterLabel">senderAddress [<?php echo $i; ?>]</div>
						<div class="ParameterValue"><?php echo $inboundSMSMessage[$i]->getSenderAddress();?></div>				
					</div>
				<?php } ?>
			<?php 
			} 
