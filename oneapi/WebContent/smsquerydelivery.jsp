...
<%@ page import="org.gsm.oneapi.responsebean.sms.SMSSendDeliveryStatusResponse"%>
<%@ page import="org.gsm.oneapi.responsebean.sms.DeliveryInfoList"%>
...
<%
	...
	SMSSendDeliveryStatusResponse smsResponse=null;
	...
	if (submit==null) {
		...
		username="Fred.Jones";
		password="1234";
		endpoint=sandboxEndpoints.getServiceEndpoints().getQuerySMSDeliveryEndpoint();

	} else {
		sandboxEndpoints.setQuerySMSDelivery(endpoint);
		...
		smsResponse=me.queryDeliveryStatus(senderAddress, requestId);	
		
		if (smsResponse!=null && smsResponse.getDeliveryInfoList()!=null) deliveryInfo=smsResponse.getDeliveryInfoList().getDeliveryInfo();
	}
%>
	...
	<%@ include file="mainmenu.inc" %>
        ...
        <div id="other-main">
          ...
          <div class="other-bottom">
            <div class="left">
          <h3>
            Outbound SMS API - Query SMS Delivery Status
          </h3>
          <p>
      
		</p>
		
		<form method="post" action="smsquerydelivery.jsp">
          ...
          <h3>Response:</h3>
			...
			<% if (smsResponse.getRequestError()!=null) { %>
				...
				<% } %>
			<% } %>
			<% if (smsResponse.getDeliveryInfoList()!=null) { %>
				...
				<% if (deliveryInfo!=null && deliveryInfo.length>0) {
	          	...
	          	<div class="ParameterRow">
					...
					<div class="ParameterValue"><%=smsResponse.getDeliveryInfoList().getResourceURL()%></div>				
				</div>
			<% } %>
